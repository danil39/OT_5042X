...
/* -------------------------------------------------------------------------- */
/* 12/04/2013|     Xian.Jiang       |       PR552304       |Can't turn on wifi*/
/*           |                      |                      |hotspot if USB tet*/
/*           |                      |                      |hering has been   */
/*           |                      |                      |connected.        */
/* ----------|----------------------|----------------------|----------------- */
...
void create_bound_listener(struct listener **listeners, struct irec *iface)
{
  ...
  //[BUGFIX]-Mod-BEGIN by TCTNB.xian.jiang,12/04/2013,PR-552304,
  //should malloc a new iface and copy the datas when new a listener.
  //new->iface = iface;
  struct irec *new_iface = safe_malloc(sizeof(struct irec));
  new_iface->addr = iface->addr;
  new_iface->netmask = iface->netmask;
  (*new_iface).dhcp_ok= (*iface).dhcp_ok;
  (*new_iface).mtu= (*iface).mtu;
  new_iface->next = iface->next;
  new->iface = new_iface;
...
void set_interfaces(const char *interfaces)
{
    ...
    while (prev_interfaces) {
      ...
      //[BugFix]Mod-Start by TCTNB.Zhang Jinbo,2014.06.21,PR692171,release by QC
      //free(prev_interfaces);
      int found = 0;
      for (new_iface = daemon->interfaces; new_iface; new_iface = new_iface->next)
      ...
      if (!found)
        free(prev_interfaces);
      //[BugFix]Mod-End by TCTNB.Zhang Jinbo,2014.06.21,PR692171,release by QC
