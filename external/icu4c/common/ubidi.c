    ...
    if(pBiDi->flags&MASK_WS) {
        ...
        while(i>0) {
            ...
            while(i>0) {
                ...
                //[BUGFIX]-Add-BEGIN by TSNJ.Anming.Wei,12/11/2013,PR-552454, [HOMO][HOMO]Arabic numbers will automatically flip once you add space
                if(flag & DIRPROP_FLAG(U_WHITE_SPACE_NEUTRAL)) {
                    UBool bDigitWord = TRUE;
                    int j = i;
                    Flags flag0;
                    while(j > 0) {
                        flag0=DIRPROP_FLAG_NC(dirProps[--j]);
                        if((flag0 & DIRPROP_FLAG(U_ARABIC_NUMBER)) || (flag0 & DIRPROP_FLAG(U_EUROPEAN_NUMBER))) {
                            continue;
                        }
                        if(!(flag0 & DIRPROP_FLAG(U_WHITE_SPACE_NEUTRAL))) {
                            bDigitWord = FALSE;
                        }
                        break;
                    }
                ...
                //[BUGFIX]-Add-END by TSNJ.Anming.Wei,12/06/2013
