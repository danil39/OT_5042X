...
std::string SysInfo::GetDeviceName() {
  ...
  char us_is_on[PROP_VALUE_MAX];
  __system_property_get("persist.sys.mmiua", us_is_on);
  if(std::string(us_is_on) == "true"){
      __system_property_get("persist.sys.uaname", device_model_str);
      LOG(ERROR) << "ua is ok";
