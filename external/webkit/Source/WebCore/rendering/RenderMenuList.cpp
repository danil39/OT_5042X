...
/* ----------|----------------------|----------------------|----------------- */
/* 10/22/2013|     wen.ye           |       PR518785     |pdp error is network problem, this patch is just used to   */
/*           |                      |                    |resolve: though switch from lte to 3G/2G, will not do pdp   */
/*           |                      |                    |activation at most time  */
/* ----------|----------------------|----------------------|----------------- */
...
//[BUGFIX]-Mod-BEGIN by TCTNB.wen.ye,10/22/2013,518785,
//just a improvement patch
...
void RenderMenuList::setTextFromOption()
{
    SelectElement* select = toSelectElement(static_cast<Element*>(node()));
    const Vector<Element*>& listItems = select->listItems();
    int size = listItems.size();
    String text = "";
    bool hasOptionStyle = false;
    int number = 0;
    for (int i = 0; i < size; i++) {
        Element* element = listItems[i];
        if (OptionElement* optionElement = toOptionElement(element)) {
            ...
            number++;
            if(hasOptionStyle == FALSE){
                m_optionStyle = element->renderStyle();
                hasOptionStyle = true;
            }else{
                text.append("; ");
            }
            String label = optionElement->textIndentedToRespectGroupLabel();
            text.append(label);
        }
    }
    int temp_width = static_cast<int>(ceilf(style()->font().width(text)));
    if(number == 1 || temp_width < m_optionsWidth || text.length() <= 3){
        setText(text.stripWhiteSpace());
        return;
    }

    String lost = "...";
    int width_lost = static_cast<int>(ceilf(style()->font().width(lost)));

    String label_add;
    String temp_label = text;
    String label = "";
    int label_add_width = 0;
    int label_width = 0;

    int width_left = m_optionsWidth - width_lost;
    int length_add = 0;
    bool isShort = false;
    bool isLong = true;
    if(width_left <= 0){
    ...
    int count = 0;
    long temp_label_add_width = temp_width;

    while((isShort || isLong) && (count < 10)){
        if(isLong){
            label_add = temp_label;
            label_add_width = temp_label_add_width;

            length_add = label_add.length() * width_left / label_add_width;
            length_add = (length_add < label_add.length()) ? length_add : (label_add.length() -1);
            if(length_add <= 0){
                temp_label = "";
                break;
            }
            temp_label = label_add.substring(0, length_add);
            temp_label_add_width = static_cast<int>(ceilf(style()->font().width(temp_label)));

        }else if(isShort){
            label.append(temp_label);
            width_left = m_optionsWidth - width_lost - static_cast<int>(ceilf(style()->font().width(label)));
            label_add = label_add.substring(length_add, label_add.length()-length_add);
            label_add_width = static_cast<int>(ceilf(style()->font().width(label_add)));
            length_add = label_add.length() * width_left / label_add_width;
            length_add = (length_add < label_add.length()) ? length_add : (label_add.length() -1);
            if(length_add <= 0){
                temp_label = "";
        ...
        isShort = width_left>temp_label_add_width;
        isLong = width_left<temp_label_add_width;
        if(isShort || isLong){
            count++;
        }
    }
